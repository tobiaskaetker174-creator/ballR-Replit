    ...
    const aside = (
      <>
        <View style={styles.webSideCard}>
          <Text style={styles.webSideLabel}>Identity</Text>
          <View style={styles.webIdentityCard}>
            <BigAvatar name={ME.name} size={78} />
            <Text style={styles.webIdentityName}>{ME.name}</Text>
            <Text style={styles.webIdentitySub}>
              {ME.nationality} · {ME.basedIn}
            </Text>
            <View style={[styles.eloChip, { backgroundColor: `${eloTier.color}22` }]}>
              <Text style={[styles.eloChipText, { color: eloTier.color }]}>
                {ME.eloRating} ELO · {eloTier.label}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.webSideCard}>
          <Text style={styles.webSideLabel}>Account Actions</Text>
          <Pressable style={styles.webActionButton} onPress={() => setShowEditProfile(true)}>
            <Feather name="edit-2" size={15} color={Colors.accent} />
            <Text style={styles.webActionButtonText}>Edit profile</Text>
          </Pressable>
          <Pressable style={styles.webActionButton} onPress={() => router.push("/analytics")}>
            <Ionicons name="bar-chart-outline" size={15} color={Colors.accent} />
            <Text style={styles.webActionButtonText}>Open analytics</Text>
          </Pressable>
          {isLoggedIn ? (
            <Pressable style={styles.webGhostButton} onPress={logout}>
              <Ionicons name="log-out-outline" size={15} color={Colors.muted} />
              <Text style={styles.webGhostButtonText}>Log out</Text>
            </Pressable>
          ) : (
            <Pressable style={styles.webActionButton} onPress={() => router.push("/auth")}>
              <Ionicons name="person-outline" size={15} color={Colors.accent} />
              <Text style={styles.webActionButtonText}>Log in</Text>
            </Pressable>
          )}
        </View>

        <View style={styles.webSideCard}>
          <Text style={styles.webSideLabel}>Community Pulse</Text>
          <View style={styles.webPulseRow}>
            <Text style={styles.webPulseLabel}>Pending reviews</Text>
            <Text style={styles.webPulseValue}>{pendingReviews.length}</Text>
          </View>
          <View style={styles.webPulseRow}>
            <Text style={styles.webPulseLabel}>Notifications</Text>
            <Text style={styles.webPulseValue}>{unreadCount}</Text>
          </View>
          <View style={styles.webPulseRow}>
            <Text style={styles.webPulseLabel}>Calibration left</Text>
            <Text style={styles.webPulseValue}>{calibrationGamesLeft}</Text>
          </View>
        </View>
      </>
    );
    ...
    return (
      <>
        <WebAppShell
          ...
          subtitle="The same player model as mobile, reformatted into a desktop profile cockpit with ratings, reliability, analytics, and community feedback."
          primaryActionLabel={isLoggedIn ? "Open Analytics" : "Log In"}
          primaryActionRoute={isLoggedIn ? "/analytics" : "/auth"}
          aside={aside}
        >
          <View style={styles.webProfileHero}>
            <View style={styles.webProfileIntro}>
              <Text style={styles.webProfileEyebrow}>Player snapshot</Text>
              <Text style={styles.webProfileTitle}>
                {ME.name} is {eloPublic ? "publicly ranked" : "still in protected rating mode"} in BallR.
              </Text>
              <Text style={styles.webProfileBody}>
                Reliability, match results, Baller Score, teammate reviews, and ELO history all stay intact from the mobile app. We are just giving them room to breathe in the browser.
              </Text>
              <View style={styles.webProfileBadgeRow}>
                <View style={styles.webProfileBadge}>
                  <Ionicons name="shield-checkmark-outline" size={14} color={reliabilityColor} />
                  <Text style={[styles.webProfileBadgeText, { color: reliabilityColor }]}>
                    {ME.reliabilityScore}% {reliabilityLabel}
                  </Text>
                </View>
                <View style={styles.webProfileBadge}>
                  <Ionicons name="flash-outline" size={14} color={eloTier.color} />
                  <Text style={[styles.webProfileBadgeText, { color: eloTier.color }]}>
                    {ME.eloRating} ELO
                  </Text>
                </View>
                <View style={styles.webProfileBadge}>
                  <Ionicons name="trophy-outline" size={14} color={Colors.amber} />
                  <Text style={[styles.webProfileBadgeText, { color: Colors.amber }]}>
                    BallR Score {ME.ballerScore ?? 0}
                  </Text>
                </View>
              </View>
            </View>

            <View style={styles.webProfileStatGrid}>
              <View style={styles.webProfileStatCard}>
                <Text style={styles.webProfileStatValue}>{ME.gamesPlayed}</Text>
                <Text style={styles.webProfileStatLabel}>Games played</Text>
                <Text style={styles.webProfileStatSub}>
                  {ME.gamesWon}W · {ME.gamesLost}L · {ME.gamesDrawn}D
                </Text>
              </View>
              <View style={styles.webProfileStatCard}>
                <Text style={[styles.webProfileStatValue, { color: reliabilityColor }]}>{ME.reliabilityScore}%</Text>
                <Text style={styles.webProfileStatLabel}>Reliability</Text>
                <Text style={styles.webProfileStatSub}>{reliabilityLabel}</Text>
              </View>
              <View style={styles.webProfileStatCard}>
                <Text style={styles.webProfileStatValue}>{winRate}%</Text>
                <Text style={styles.webProfileStatLabel}>Win rate</Text>
                <Text style={styles.webProfileStatSub}>{ME.avgSportsmanshipRating.toFixed(1)} spirit</Text>
              </View>
            </View>
          </View>

          <View style={styles.webProfileColumns}>
            <View style={styles.webMainStack}>
              {ME.bio ? (
                <View style={styles.webPanel}>
                  <Text style={styles.webPanelLabel}>Player bio</Text>
                  <Text style={styles.webPanelBody}>{ME.bio}</Text>
                </View>
              ) : null}

              <View style={styles.webPanel}>
                <Text style={styles.webPanelLabel}>Performance band</Text>
                <EloRangeVisual currentElo={ME.eloRating} onInfoPress={() => setShowEloInfo(true)} />
              </View>

              <View style={styles.webPanel}>
                <Text style={styles.webPanelLabel}>Profile details</Text>
                <View style={styles.webInfoGrid}>
                  ...
                  <View style={styles.webInfoCard}>
                    <InfoRowItem label="Favorite Team" value={ME.favoriteTeam ?? "Not set"} />
                  </View>
                  <View style={styles.webInfoCard}>
                    <InfoRowItem label="Plays Like" value={ME.favoritePlayer ?? "Not set"} />
                  </View>
                </View>
              </View>
              ...
              {eloPublic ? (
                <View style={styles.webPanel}>
                  <Text style={styles.webPanelLabel}>ELO history</Text>
                  <View style={styles.webHistoryList}>
                    {ELO_HISTORY.slice(0, 6).map((entry, index) => (
                      <View
                        key={index}
                        style={[
                          styles.webHistoryRow,
                          { borderBottomWidth: index === 5 ? 0 : 1 },
                        ]}
                      >
                        <Text style={styles.webHistoryVenue}>{entry.venueName}</Text>
                        <Text
                          style={[
                            styles.webHistoryDelta,
                            { color: entry.change > 0 ? Colors.accent : entry.change < 0 ? Colors.red : Colors.amber },
                          ]}
                        >
                          {entry.change > 0 ? `+${entry.change}` : entry.change}
                        </Text>
                        <Text style={styles.webHistoryFinal}>{entry.eloAfter}</Text>
                      </View>
                    ))}
                  </View>
                </View>
            ...
            <View style={styles.webSecondaryStack}>
              <View style={styles.webPanel}>
                <Text style={styles.webPanelLabel}>Reviews</Text>
                {acceptedReviews.length > 0 ? (
                  acceptedReviews.map((review) => (
                    <View key={review.id} style={styles.webReviewCard}>
                      <Text style={styles.webReviewAuthor}>{review.author.name}</Text>
                      <Text style={styles.webReviewTime}>{formatTimestamp(review.createdAt)}</Text>
                      <Text style={styles.webReviewText}>{review.text}</Text>
                    </View>
                  ))
                ) : (
                  <Text style={styles.webPanelBody}>No accepted reviews yet.</Text>
                )}
              </View>

              <View style={styles.webPanel}>
                <Text style={styles.webPanelLabel}>Social links</Text>
                <View style={styles.webProfileBadgeRow}>
                  {ME.instagram ? (
                    <View style={styles.webProfileBadge}>
                      <Ionicons name="logo-instagram" size={14} color={Colors.purple} />
                      <Text style={[styles.webProfileBadgeText, { color: Colors.purple }]}>@{ME.instagram}</Text>
                    </View>
                  ) : null}
                  {ME.whatsapp ? (
                    <View style={styles.webProfileBadge}>
                      <Ionicons name="logo-whatsapp" size={14} color={Colors.accent} />
                      <Text style={[styles.webProfileBadgeText, { color: Colors.accent }]}>{ME.whatsapp}</Text>
                    </View>
                  ) : null}
                  {!ME.instagram && !ME.whatsapp ? (
                    <Text style={styles.webPanelBody}>No public social links yet.</Text>
                  ) : null}
                </View>
              </View>
            </View>
